...
Construire un home page marketing pour **MultiForme Le Gym** (Centre de conditionnement physique, Sorel-Tracy).
...
Avant d'écrire la moindre ligne de code dans `components/sections/home/*`, écris `decisions.md` à la racine du repo. Ce fichier contient trois blocs courts et spécifiques à **MultiForme Le Gym** :
...
Avant de figer la palette / typo / vibe dans `app/globals.css` ou `tailwind.config.ts`, écris `theme-variants.md` à la racine du repo. Ce fichier expose **3 directions distinctes** que tu as considérées :
...
Avant de lancer `npm run build`, parcours le repo et vérifie qu'AUCUN des patterns suivants n'est présent :

- [ ] **Aucun `@ts-nocheck`** dans le code source. Interdit. Si TypeScript râle, fix la vraie erreur (le plus souvent : ajouter `'use client'` en première ligne d'un fichier qui utilise `useState` / `useEffect` / autre hook React).
...
- [ ] `npx tsc --noEmit` (ou `npm run typecheck` si exposé) passe sans erreur.
...
5. Implémente les tokens du thème dans `app/globals.css` et `tailwind.config.ts` selon la direction retenue. Utilise EXCLUSIVEMENT les couleurs de `lead.json.palette` + neutres slate/zinc documentés.
6. Pour CHAQUE section listée plus bas : d'abord `content/section-designs/<Component>.design.md` (discipline 3), ensuite `components/sections/home/<Component>.tsx`.
7. Wire `app/page.tsx` comme une composition root mince (≤30 lignes), important les sections.
...
10. `npm run build` doit passer sans erreur.
11. Self-review final (discipline 7).
12. Commit + push to `main` avec les commandes ci-dessous.

## Sections à inclure

- `Hero` — composant à créer dans `components/sections/home/Hero.tsx`
- `About` — composant à créer dans `components/sections/home/About.tsx`
- `Services` — composant à créer dans `components/sections/home/Services.tsx`
- `Gallery` — composant à créer dans `components/sections/home/Gallery.tsx`
- `Contact` — composant à créer dans `components/sections/home/Contact.tsx`
- `Footer` — composant à créer dans `components/sections/home/Footer.tsx`
...
- **`use client`** sur tout fichier qui utilise `useState`, `useEffect`, ou un autre hook React.
- **Pas de `@ts-nocheck`** — corrige la vraie erreur (souvent : `'use client'` manquant).
